...
import { Alert, AlertTitle, Button, Card, CardContent, Chip, Divider, Grid, IconButton, InputLabel, Link, Modal, OutlinedInput, Stack, TextField, Typography } from '@mui/material';
import { useCallback, useContext, useEffect, useRef, useState } from 'react';
import { console_log, empty, getTicketAttachmentUrl, get_data_value, get_utc_timestamp_ms, priceFormat, timeConverter, ValidEmail } from 'utils/misc';
import { showToast } from 'utils/utils';
import { useDispatch, useSelector } from 'react-redux';
import MainCard from 'components/MainCard';
import { apiAdminUpdateUserInfo } from 'services/adminService';
import { ClockCircleOutlined, CloudDownloadOutlined, CommentOutlined, EditOutlined, InfoCircleOutlined, SendOutlined, UserOutlined, WarningFilled } from '@ant-design/icons';
import UploadSingleFile from 'components/third-party/dropzone/SingleFile';
import { APP_NAME, ENVIRONMENT } from 'config/constants';
import { apiAdminGetTicketDetail, apiAdminSubmitTicketMessage } from 'services/adminTicketService';
...
const TicketDetailModal = (props) => {
    ...
    const loadTicketInfo = async (id) => {
        ...
        const apiRes = await apiAdminGetTicketDetail(payload)
        setApiCalling(false)
        if (apiRes['status'] === '1') {
    ...
    const handleSubmit = async (e) => {
        ...
        setApiCalling(true)
        const apiRes = await apiAdminSubmitTicketMessage(payload, uploadFile)
        setApiCalling(false)
        if (apiRes['status'] === '1') {
            const ticket_info = apiRes?.data?.ticket_info
            setRow(ticket_info)
                                                        ...
                                                        <Grid item xs={12} key={index}>
                                                            {history.sender_id === 0 ? (
                                                                ...
                                                                <Stack direction="row" spacing={1.25} alignItems="flext-start">
                                                                    <UserAvatar user={{ avatar: '', name: history?.sender_name }} />
                                                                    <Grid container>
