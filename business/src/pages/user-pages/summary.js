...
import { apiUserGetAllStats } from 'services/userStoreService';
import { showToast } from 'utils/utils';
import { copyObject, formatDate, get_data_value, get_utc_timestamp_ms, is_empty } from 'utils/misc';
...
const Summary = () => {
  ...
  const loadPageData = async () => {
    ...
    const apiRes = await apiUserGetAllStats(payload)
    setLoading(false)
    ...
    if (apiRes['status'] === '1') {
      ...
      if (errorData['store_integration_error'] === '1') {
        setPageData(errorData)
      }
      if (errorData['invalid_store_permission'] === '1') {
        const settingData = {
