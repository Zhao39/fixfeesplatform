...
import { console_log, empty, get_data_value, get_utc_timestamp, get_utc_timestamp_ms, timeConverter } from 'utils/misc';
import { useDispatch, useSelector } from 'react-redux';
import { useNavigate } from 'react-router';
import { apiUserReadNotification } from 'services/userNotificationService';
import { SocketContext } from 'contexts/socket';
import { apiAcceptStoreMemberInvitation, apiRejectStoreMemberInvitation } from 'services/userStoreService';
...
const NotificationMenuItem = (props) => {
  const { item = {}, history, setOpen, readUserNotification } = props
  ...
  const onClickNotificationItem = () => {
    ...
    if (data_type === 'ticket') {
      ...
      const redirectUrl = `/user/support/ticket?id=${ticketId}`
      history(redirectUrl)
    }

    readUserNotification(item.id)
    setOpen(false)
  }
  ...
  const onClickYes = async () => {
    const data = item.data
    const data_type = item.data_type
    if (data_type === 'store_team_invite') {
      const dataObj = data.split('_')
      const [teamMemberId, userId, inviterId, storeId] = dataObj
      console_log(`teamMemberId, userId, inviterId, storeId::::`, teamMemberId, userId, inviterId, storeId)
      await apiAcceptStoreMemberInvitation({ id: teamMemberId })

      /////////////////////////////////////////// refresh store list////////////////////////////////////////////////////////
      const settingData = {
        storeUpdatetimestamp: get_utc_timestamp_ms()
      }
  ...
  const onClickNo = async () => {
    ...
    if (data_type === 'store_team_invite') {
      ...
      const [teamMemberId, userId, inviterId, storeId] = dataObj
      console_log(`teamMemberId, userId, inviterId, storeId::::`, teamMemberId, userId, inviterId, storeId)
      await apiRejectStoreMemberInvitation({ id: teamMemberId })
    }
...
const Notification = () => {
  ...
  const getNotificationList = () => {
    ...
    const notification_list = socketDataStore?.user_notification_data?.notification_list
  ...
  const readUserNotification = (id) => {
    socket.emit("read_user_notification", {
      ...socketHeader,
      id: id,
                      ...
                      notificationList.map((item, index) => {
                        return (
                          <NotificationMenuItem
                            ...
                            history={history}
                            setOpen={setOpen}
                            readUserNotification={(id) => readUserNotification(id)}
                          />
                        )
                      })
                    }
