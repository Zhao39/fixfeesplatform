...
import { apiCheckCoupon } from 'services/authService';
...
const BillingForm = (props) => {
    ...
    const onSubmitFormData = async (values) => {
        ...
        if (couponApplied) {
            regData['coupon'] = couponInfo?.name
        } else {
            regData['coupon'] = ""
        }
    ...
    const onClickApplyCoupon = async (couponCode) => {
        console.log(`onClickApplyCoupon:::`, couponCode)
        if (isSubmittingCoupon || couponApplied) {
            return false
        }
        if (couponCode) {
            ...
            console.log(`onClickApplyCoupon params:::`, params)
            const apiRes = await apiCheckCoupon(params)
            setIsSubmittingCoupon(false)
            if (apiRes['status'] === '1') {
                setCouponInfo(apiRes['data'])
    ...
    const getCouponAppliedPrice = (price) => {
        let appliedPrice = price
        if (couponApplied && couponInfo) {
            const coupon_type = Number(couponInfo?.type)
            const coupon_value = Number(couponInfo?.value)
            if (coupon_type === 0) { // x-day trial coupon
                appliedPrice = 1
            }
            if (coupon_type === 1) { // percentage coupon
                appliedPrice = price * coupon_value / 100
                appliedPrice = Number(appliedPrice.toFixed(2))
            }
        }
        return appliedPrice
    }
                                            ...
                                            <Grid item xs={12} sm={12}>
                                                <Stack spacing={1.25}>
                                                    ...
                                                    <Stack direction={`row`} spacing={1.25}>
                                                        ...
                                                        />
                                                        <Button
                                                            disabled={!values.coupon || isSubmittingCoupon || couponApplied}
                                                            variant="contained"
                                                            color="primary"
                                                            type="button"
                                                            onClick={() => onClickApplyCoupon(values.coupon)}
                                                        >
                                                            Apply
                                                        </Button>
                                                    </Stack>
                                                ...
                                                <Stack direction={`column`} spacing={3}>
                                                    ...
                                                    <Stack direction={`row`} justifyContent={`space-between`} alignItems={`center`} spacing={3}>
                                                        ...
                                                        <Button disableElevation disabled={isSubmitting || submittingDisabled} size="large" type="submit" variant="contained" color="primary">
                                                            {`Pay ( $${getCouponAppliedPrice(amount)} ) & Register`}
                                                        </Button>
                                                    </Stack>
                                                </Stack>
                                            </>
