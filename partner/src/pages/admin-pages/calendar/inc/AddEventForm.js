...
import { apiAdminDeleteCalendarEvent, apiAdminSaveCalendarEvent } from 'services/adminCalendarService';
...
const AddEventFrom = (props) => {
  ...
  const deleteHandler = async () => {
    if (window.confirm('Are you sure?')) {
      ...
      await apiAdminDeleteCalendarEvent(payload)
      showToast("Event has been deleted successfully", "success")
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        await apiAdminSaveCalendarEvent(newEvent)
        setSubmitting(false);
                  ...
                  <Grid item xs={12}>
                    <FormControl>
                      <RadioGroup
                        ...
                        onChange={(e) => setFieldValue('colorId', e.target.value)}
                        name="color-radio-buttons-group"
                        sx={{ '& .MuiFormControlLabel-root': { mr: 2 } }}
                      >
                        {eventColors.map((item, index) => (
                          <ColorPalette key={index} value={item.id} color={item.background} />
                        ))}
                      </RadioGroup>
                    </FormControl>
                  </Grid>
