...
import { getStatusTextColor } from "config/global_functions";
  ...
  const renderActionCell = (c_props) => {
    ...
    return (
      <Stack direction={`row`} spacing={1}>
        <IconButton title="View" style={{ color: getStatusTextColor(row.status, mode) }} variant="outlined" size="small" onClick={() => onClickViewLead(row)}>
          <EyeOutlined />
        </IconButton>
