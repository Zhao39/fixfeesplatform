...
import { get_data_value, timeConverter } from 'utils/misc';
...
function ReactTable({ columns, data, getHeaderProps, handleAdd }) {
  const theme = useTheme();
  const matchDownSM = useMediaQuery(theme.breakpoints.down('sm'));

  const filterTypes = useMemo(() => renderFilterTypes, []);
  const sortBy = { id: 'add_timestamp', desc: true };
  ...
  const {
  ...
  } = useTable(
    {
      ...
      filterTypes,
      // @ts-ignore
      initialState: { pageIndex: 0, pageSize: 5, hiddenColumns: ['id', 'email'], sortBy: [sortBy] }
    },
    useGlobalFilter,
    useFilters,
    useSortBy,
...
const PixelInstallLogsSection = (props) => {
...
export default PixelInstallLogsSection;