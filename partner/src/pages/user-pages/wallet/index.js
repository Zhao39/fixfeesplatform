...
import { curl_post, get_data_value, get_utc_timestamp_ms, priceFormat } from 'utils/misc';
...
import { urlUserExportWalletPageData } from 'services/constants';
...
const WalletPage = (props) => {
  ...
  const onClickRequestWithdrawal = () => {
    if (user.kyc_status === 1) {
      ...
      showToast(`Your KYC documents are still pending approval. Access to the wallet will be available once your KYC is approved by the admin.`, 'error')
    }
  }
  ...
  const onClickExport = async () => {
    ...
    curl_post(urlUserExportWalletPageData, payload, 'post')
  }
