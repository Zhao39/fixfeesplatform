...
const UserPaymentListTable = (props) => {
  ...
  const { handleDownloadInvoice } = props
  ...
  const TABLE_COLUMNS = [
    ...
    },
    {
      Header: "Invoice",
      accessor: "type",
      Cell: (c_props) => renderInvoiceCell(c_props)
    },
    {
      Header: "Created at",
      accessor: "created_at",
  ...
  const renderInvoiceCell = (c_props) => {
    const row = c_props.row.original;
    return (
      <span><Button variant="outlined" size="small" onClick={() => {
    ...
    <>
      <ScrollX>
        <DataTableWrapper
          ...
          dataListApiUrl={`${BASE_API_URL}/user/transaction/get-data-list`}
          handleAdd={handleAdd}
