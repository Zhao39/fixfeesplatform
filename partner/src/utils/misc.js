...
export const priceFormat = (num, currencySign = null) => {
	...
	if (currencySign) {
		if (num < 0) {
			num = (-1) * Number(num)
			num_formatted = num.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1,");
...
export const getPreviousMonthYear = (currentMoment, format = "MMMM YYYY") => {
	if (empty(currentMoment)) {
		currentMoment = moment()
	}
	const previousMonth = currentMoment.clone().subtract(1, 'months');
	// const monthNumber = previousMonth.month() + 1; // 1-12
	const monthYearName = previousMonth.format(format);
	return monthYearName
}

export const getNextMonthYear = (currentMoment, format = "MMMM YYYY") => {
	if (empty(currentMoment)) {
		currentMoment = moment()
	}
	const nextMonth = currentMoment.clone().add(1, 'months');
	const monthYearName = nextMonth.format(format);
	return monthYearName
}

export const addZeroPrefix = (str, length = 2) => {
	str = `00000000${str}`
...
export const getUnixtimestampFromUTCDate = (isoDateString) => {
	var t = moment.utc(isoDateString).unix()
	return t
}
