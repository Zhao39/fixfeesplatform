...
const ComponentsLayout = ({ handleDrawerOpen, componentDrawerOpen }) => {
  ...
  return (
    <Box sx={{ display: 'flex', pt: componentDrawerOpen ? { xs: 0, md: 3, xl: 5.5 } : 0, background: 'linear-gradient(90deg, #3BC734 0%, #088E01 80%)' }}>
      <Drawer handleDrawerOpen={handleDrawerOpen} open={componentDrawerOpen} />
      <Main theme={theme} open={componentDrawerOpen} style={{ background: 'linear-gradient(90deg, #3BC734 0%, #088E01 80%)'}}>
        <Outlet />
      </Main>
    </Box>
  );
