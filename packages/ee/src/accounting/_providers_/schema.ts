...
// Accounting provider-agnostic interfaces
// These are the uniform interfaces that providers (Xero, QuickBooks, etc.) should deserialize to
export namespace Accounting {
  // Uniform Customer interface - deserialized from provider-specific formats
  export interface Customer {
    // Core identification
    id: string; // External ID from the accounting provider
    ...
    syncToken?: string; // For optimistic locking (QuickBooks)
    customFields?: Record<string, any>;

    // Timestamps (provider format)
    createdAt?: string;
    updatedAt?: string;
  }

  // Uniform Vendor interface - deserialized from provider-specific formats
