...
import { console_log, get_data_value, intval, number_format } from "../helpers/misc";
...
export default class PdfCreator {
    ...
    public create_invoice_pdf = async (item: any) => {
        ...
        let invoice_file_name = "FixFeesPlatform-Invoice-" + invoice_id + ".pdf";
        let invoice_path = "assets/global/invoice/" + invoice_file_name;
        let logo_path = "assets/global/img/logo.png";
        let paid_img_path = "assets/global/img/paid-img.png";
        if (ASSETS_DIR !== "") {
            invoice_path = ASSETS_DIR + "/global/invoice/" + invoice_file_name;
            logo_path = ASSETS_DIR + "/global/img/logo.png";
            paid_img_path = ASSETS_DIR + "/global/img/paid-img.png";
        }

        if (false && fs.existsSync(invoice_path)) {
            // path exists
            console_log("invoice exists:", invoice_path);
        ...
        curY += 30;
        doc.fontSize(subTitleFontSize).font(mainFontBold).fillColor(mainFontColor).text("Fix My Fees", pagePaddingX, curY, {
            width: pageLeftSideWidth,
            align: 'left'
        });
        curY += 24;
        doc.fontSize(smallFontSize).font(mainFont).fillColor(mainFontColor).text("16192 Coastal Highway, ", pagePaddingX, curY, {
            width: pageLeftSideWidth,
            align: 'left'
        });
        curY += smallFontSize + 3;
        doc.fontSize(smallFontSize).font(mainFont).fillColor(mainFontColor).text("Lewes DE 19958", pagePaddingX, curY, {
            width: pageLeftSideWidth,
            align: 'left'
        });
        curY += smallFontSize + 3;
        doc.fontSize(smallFontSize).font(mainFont).fillColor(mainFontColor).text("Website: partners.fixmyfees.com", pagePaddingX, curY, {
            width: pageLeftSideWidth,
            align: 'left',
            link: 'https://partners.fixmyfees.com/',
            underline: true
        });
        ...
        curY += 24;
        let description = item['trans_type'];
        // if (item['description'].indexOf("crypto_license") > -1) {
        //     description = "HLX Crypto";
        // } else if (item['trans_type'] === 'Membership') {
        //     let exp_days = 7;
        ...
        // } else if (item['trans_type'] === 'Membership Smart Pack' || item['trans_type'] === 'Membership Smart Pack (Rebill)') {
        //     description = APP_NAME + " Membership Annual - " + "Full Access to Educational Videos and software";
        // } else if (item['trans_type'] === 'Discounted Membership' || item['trans_type'] === 'Discounted Membership (Rebill)') {
        //     description = APP_NAME + " Discounted Membership - " + "Full Access to Educational Videos and software";
        // }
        ...
        curY += 24;
        let transaction_amount = item['paid_amount']
    ...
    public create_invoice_pdf2 = async (invoice_id: string, invoice_type: string = "membership") => {
        ...
        curY += 40;
        doc.fontSize(subTitleFontSize).font(mainFontBold).fillColor(mainFontColor).text("FixFeesPlatform", pagePaddingX, curY, {
            width: pageLeftSideWidth,
            align: 'left'
        });
        curY += 24;
