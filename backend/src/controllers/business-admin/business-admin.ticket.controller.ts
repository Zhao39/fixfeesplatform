...
export default class BusinessAdminTicketController extends BusinessAdminBaseController {
  constructor() {
    super();
  }
  ...
  public submitTicket = async (req: Request, res: Response) => {
    try {
      ...
      let receiver_id = post_param['receiver_id']
      let receiverInfo = await businessService.getOne({ id: receiver_id })
      let update_data = {
        title: title,
        description: description,
        sender_id: receiver_id,
        sender_name: receiverInfo['name'],
        receiver_id: 0,
        receiver_name: 'Admin',
      ...
      const notificationId = await userNotificationService.addUserNotification(update_data['receiver_id'], notificationRow)

      //await this._sendTicketEmailToUser(user)
      return this.json_output_data(data, "New ticket has been submitted successfully!", res);
    } catch (e) {
      console.log("error:::::::::", e)
      return this.json_output_error("", "", res)
    }
...
export const businessAdminTicketController = new BusinessAdminTicketController()
