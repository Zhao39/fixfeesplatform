import { empty, get_utc_timestamp } from "../helpers/misc";
import { TB_USER_NOTIFICATION } from "../var/tables";
...
export default class UserNotificationService extends BaseService {
  constructor() {
    super();
    this.tableName = TB_USER_NOTIFICATION;
  }
  ...
  public addUserNotification = async (userId, row) => {
    try {
      ...
      if (item['user_id'] > 0) {
        socketInstance.reloadUserNotification(item['user_id'])
      }
  ...
  public getUserNotificationList = async (userId, status = "", environment = null) => {
    try {
      ...
      const currentTimestamp = get_utc_timestamp()
      let sql = `select id from ${TB_USER_NOTIFICATION} where 1=1`
  ...
  /**
   * read a notfication for a user
   */  
  public readUserNotification = async (userId, notificationId = null, environment = null) => {
    try {
      if (environment === null) {
        environment = ENVIRONMENT.PARTNER
      }
      ...
      socketInstance.reloadUserNotification(userId)
  ...
  /**
   * read a user ticket notification
   */  
  public readUserTicketNotification = async (userId, ticket_id, ticket_msg_id = "") => {
    try {
      ...
      let sql = `update ${TB_USER_NOTIFICATION} set status='${USER_NOTIFICATION_STATUS.READ}', update_timestamp=${currentTimestamp} where ` + condition
